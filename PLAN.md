...
3. **투자자 페르소나 온보딩** ⏳
   - 8가지 투자 특성 1~5점 자가 평가 UI
   - 특성: 스윙매매, 장기투자, 스캘핑, 우량주, ETF, 소형주, 테크주, 배당주
   - 페르소나 저장 (localStorage / Supabase)

4. **데이터 수집 파이프라인** 🔄
   - ✅ Supabase DB + Vercel Cron 구조 완성
   - ✅ mock-data.ts → DB 저장 흐름 구현
   - ⏳ Finnhub API 연동 (GitHub Actions 배치)
     - GitHub Actions cron으로 일/주/월 배치 실행
     - 종목 등급제 (A/B/C) 차등 수집
     - 상세: `docs/api-reference.md` → 데이터 수집 설계

5. **SEC EDGAR 데이터 수집** ⏳
   - 10-K / 10-Q / 8-K 원문 자동 수집
   - 보유종목 관련 공시 발생 시 브리핑에 반영

6. **LLM 브리핑 생성** ✅
   - Gemini 1.5 Flash 연동 (`@google/generative-ai`)
   - 포트폴리오 기반 개인화 프롬프트
   - 선제적 제안 생성
   - ⏳ 페르소나 기반 보완적 관점 판단 엔진
...
| Phase | 상태 | 비고 |
|-------|------|------|
| Phase 1 | 🔄 진행 중 | 페르소나 온보딩 + 댓글 시스템 + Finnhub 연동 + SEC EDGAR 수집 남음 |
