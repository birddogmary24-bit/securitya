...
종목을 3등급으로 분류하여 수집 빈도와 범위를 차등 적용.

| 등급 | 기준 | 예상 종목 수 | 수집 범위 |
|------|------|-------------|----------|
| **A (핫)** | 대형 인기주, 사용자 보유 비중 상위 | ~50 | quote + news |
| **B (일반)** | 중형주, 일반 관심 종목 | ~200 | quote + news |
| **C (기본)** | 나머지 커버리지 종목 | ~750 | quote만 |

> 등급 기준은 `stock_tickers` 테이블의 `tier` 필드로 관리. 초기에는 수동 설정, 추후 사용자 조회 빈도 기반 자동 승급/강등 고려.
...
### 배치 실행 전략: GitHub Actions

Vercel Hobby 제약(함수 60초, cron 2개)으로 대량 수집 불가.
**GitHub Actions**로 배치 실행, Supabase에 직접 저장.
...
| Job | 스케줄 (UTC) | 대상 | 예상 calls | 소요 |
|-----|-------------|------|-----------|------|
| `collect-daily` | `0 21 * * 1-5` (평일 06:00 KST) | quote + news + earnings | ~1,251 | ~21분 |
| `collect-weekly` | `0 22 * * 1` (월요일 07:00 KST) | recommendation + metrics + eps | ~150 | ~3분 |
| `collect-monthly` | `0 22 1 * *` (매월 1일 07:00 KST) | profile | ~1,000 | ~17분 |

> 월간 GitHub Actions 사용량: (21×22) + (4×3) + (1×17) = **~491분/월** (2,000분 한도의 25%)
...
steps:
  1. Setup Node.js
  2. Install dependencies (supabase-js, node-fetch)
  3. Run collect script
     - Finnhub API 호출 (1 call/sec 딜레이)
     - Supabase에 직접 upsert/insert
  4. 결과 로그 출력
```

환경 변수: GitHub repo Settings → Secrets에 `FINNHUB_API_KEY`, `SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY` 설정.

### DB 스키마 (확장)

| 테이블 | 용도 | 갱신 주기 | 주요 필드 |
|--------|------|----------|----------|
| `stock_tickers` | 종목 마스터 + 등급 | 수동/월1 | ticker, name, tier(A/B/C), sector |
| `stock_quotes` | 가격 정보 (기존) | 매일 | ticker, price, change, change_percent, previous_close |
| `stock_news` | 종목별 뉴스 (기존) | 매일 | tickers[], title, summary, source, url, sentiment |
| `stock_profiles` | 기업 프로필 **(신규)** | 월 1회 | ticker, name, logo, industry, market_cap, ipo_date |
| `earnings_calendar` | 어닝 일정 **(신규)** | 매일 | symbol, date, eps_estimate, eps_actual, revenue_estimate |
| `stock_metrics` | 재무지표 + 투자의견 **(신규)** | 주 1회 | ticker, pe_ratio, recommendations, eps_surprises |
...
| **애널리스트 목표가** (Price Target) | ❌ 미구현 | Free 플랜 미제공, 개발 스코프 외 |
